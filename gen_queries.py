...
provider_name_to_id = {
    'bareis': {'compsproxy_vendortype': 'BAREIS', 'provider_id': 4, 'provider_name': 'BAREIS'},
    'sbaor': {'compsproxy_vendortype': 'BAREIS', 'provider_id': 4, 'provider_name': 'BAREIS'},
    'crmls': {'compsproxy_vendortype': 'CRMLS', 'provider_id': 2, 'provider_name': 'CRMLS'},
    'metrolist': {'compsproxy_vendortype': 'METROLIST', 'provider_id': 7, 'provider_name': 'METROLIST'},
    'sfar': {'compsproxy_vendortype': 'SFAR_MLS', 'provider_id': 3, 'provider_name': 'SFAR'},
    'bayeast': {'compsproxy_vendortype': 'BAYEAST', 'provider_id': 9, 'provider_name': 'BAYEAST'},
    'mls_listings': {'compsproxy_vendortype': 'MLS_LISTINGS', 'provider_id': 1, 'provider_name': 'MLSL'},
    'sdmls': {'compsproxy_vendortype': 'SANDICOR', 'provider_id': 5, 'provider_name': 'SDMLS'}
}
...
and length(city)>3 
and lower(office_name) not in ('reali', 'reali, inc.', 'reali inc','reali, inc')
...
def common_joind_filters_short(table):
    return """ 
p.mls_update_date < '{0}' :: timestamp
and p.mls_update_date > '2017-01-01 00:00:00' :: timestamp
...
and p.state='CA'
and lower(p.office_name) not in ('reali', 'reali, inc.', 'reali inc','reali, inc')
and (p.is_residential_lease <> 'Y' or p.is_residential_lease is null)
""".format(timestamp_hours)


def common_joind_filters_short_no_date(table):
    return """ 
p.display_address is not null 
and p.display_address <> '' 
and p.street_number is not null 
and p.street_number not in ('',' ','0') 
and p.listing_price >= 30000::money 
...
vendors_queries = {
    'wn_crmls':
        ...
        "and ("
        "(lower(property_sub_type) in ('studio', 'condominium', 'loft', 'single family residence', 'manufactured on land','manufactured home', 'townhouse','stock cooperative') and (total_bedrooms>0 or total_bathrooms>0) "
        "or lower(property_sub_type) in ('duplex', 'multi family', 'triplex', 'quadruplex')"
        ")"
        "or ((lower(property_sub_type) is null "
    ...
    'wn_bareis':
        ...
        "or (lower(property_type) in ('condo/coop', 'single family residence', 'floating home') and (total_bedrooms>0 or total_bathrooms>0))) "
    ...
    'wn_sfar':
        ...
        "and listing_status in ('Contingent - Show','Pending','Active','Act Cont Probate', 'Act Cont Rel Clause', 'Act Cont Show','Act Cont No Show', 'Act Cont Short Sale')",
    ...
    'wn_bay_east':
        "and (lower(property_type) in ('residential income') or (lower(property_type) in ('residential') and (total_bedrooms>0 or total_bathrooms>0))) "
        "and lower(property_sub_type) in ('townhouse','condo', 'loft', 'flats', 'detached', 'duet', 'patio home/villa', 'floating homes', 'duplex', 'triplex', 'fourplex', '2houses-1lot') "
    ...
    'wn_mls_listings':
        ...
        "and ((listing_status in ('Active', 'Contingent', 'Pending (Do Not Show)') "
        "or (listing_status in ('Pending (Do Not Show)') and (display_address is null or display_address = ''))))"
}
...
vendors_join_filters_short_no_date = {
    'wn_crmls':
        common_joind_filters_short_no_date('ca_mrmls_properties') + vendors_queries['wn_crmls'],

    'wn_bareis':
        common_joind_filters_short_no_date('ca_bareis_properties') + vendors_queries['wn_bareis'],

    'wn_sfar':
        common_joind_filters_short_no_date('ca_sfar_properties') + vendors_queries['wn_sfar'],

    'wn_santa_barbara':
        common_joind_filters_short_no_date('ca_sbaor_properties') + vendors_queries['wn_santa_barbara'],

    'wn_metrolist':
        common_joind_filters_short_no_date('ca_metrolist_properties') + vendors_queries['wn_metrolist'],

    'wn_fresno':
        common_joind_filters_short_no_date('ca_fmls_properties') + vendors_queries['wn_fresno'],

    'wn_bay_east':
        common_joind_filters_short_no_date('ca_bear_properties') + vendors_queries['wn_bay_east'],

    'wn_mls_listings':
        common_joind_filters_short_no_date('ca_mlslistings_properties') + vendors_queries['wn_mls_listings'],
}
...
and {0}.listing_price >= 30000::money 
and {0}.state='CA'
and lower({0}.office_name) not in ('reali', 'reali, inc.', 'reali inc','reali, inc')
and ({0}.is_residential_lease <> 'Y' or {0}.is_residential_lease is null)
""".format(table, timestamp_months, timestamp_hours)
...
and lower(properties.office_name) not in ('reali', 'reali, inc.', 'reali inc','reali, inc')
...
vendors_queries = {
    ...
    'wn_sfar':
        "and (lower(property_sub_type) in ('2 units', '3 units', '4 units') or (lower(property_sub_type) in ('single-family homes','condominium', 'loft condominium', 'tenancy in common', 'stock cooperative') and (total_bedrooms>0 or total_bathrooms>0))) "
        "and listing_status in ('Contingent - Show','Pending','Active', 'Active Court Appr.', 'Active Court Cont.', 'Active Rel. Clause', 'New','Pending', 'Pending Sale', 'Act Cont Probate', 'Act Cont Rel Clause', 'Act Cont Show','Active Under Contract')",
    ...
    'wn_bay_east':
        "and (lower(property_type) in ('residential income') or (lower(property_type) in ('residential') and (total_bedrooms>0 or total_bathrooms>0))) "
        "and lower(property_sub_type) in ('townhouse','condo', 'loft', 'flats', 'detached', 'duet', 'patio home/villa', 'floating homes', 'duplex', 'triplex', 'fourplex', '2houses-1lot', 'townhouse') "
        "and listing_status in ('Active', 'Back on Market', 'New', 'Price Change', 'Active-REO', 'Active-Short Sale', 'Back on Market-REO', 'Back on Market-Short Sale', 'New-REO', 'New-Short Sale', 'Price Change-REO', 'Price Change-Short Sale', 'Pending', 'Pending CourtConfirmation', 'Pending Show for Backups', 'Pending Subj LenderApprov', 'Pending-REO', 'Pending Show Backups-REO', 'Pending Show Backup-Short', 'Active - Contingent') "
        "and lower(originatingsystemname) not in ('mlslisitngs','mlslistings datashare')",
...
def get_query(test_type, vendor, ids={}, start_date_in=None, end_date_in=None):
    ...
    query_test_types = {
        'properties':
            ...
            "on p.property_id = pa.property_id where {1}) t" .format(vendors_map[vendor], vendors_join_filters_short[vendor]),
        'properties_start_end_date':
            "select row_to_json(t) from (select p.property_id, city from {0}_properties p inner join {0}_property_attributes pa "
            "on p.property_id = pa.property_id where {1} and p.mls_update_date > '{2}' :: timestamp and p.mls_update_date < '{3}' :: timestamp) t".format(vendors_map[vendor], vendors_join_filters_short_no_date[vendor], start_date_in, end_date_in),
        'properties_start_date':
            "select row_to_json(t) from (select p.property_id, city from {0}_properties p inner join {0}_property_attributes pa "
            "on p.property_id = pa.property_id where {1} and p.mls_update_date > '{2}' :: timestamp) t".format(vendors_map[vendor], vendors_join_filters_short_no_date[vendor], start_date_in),
        'properties_24':
            "select row_to_json(t) from (select p.property_id, city from {0}_properties p inner join {0}_property_attributes pa "
            "on p.property_id = pa.property_id where {1} and p.mls_update_date > '{2}' :: timestamp) t".format(vendors_map[vendor], vendors_join_filters_short[vendor], str(datetime.datetime.now() - datetime.timedelta(hours=24))),
        'properties_':
        ...
        'private_remarks_all': "select row_to_json(t) from(select ca.property_id "
                               "from {0}_properties ca left join {0}_property_attributes pa "
                               "on ca.property_id = pa.property_id where ca.listing_status ='Active') t".format(vendors_map[vendor]),
        'properties_count':
        ...
        'open_house': "select row_to_json(t) from(select distinct mlnumber,fromdate, todate, fromtime, totime, listprice, status "
                      ...
                      "and {3} order by fromdate) t".format(DB_WN['collection'], vendors_map[vendor], yesterday, vendors_join_filters[vendor]),
        'open_house_1': "select row_to_json(res) from(select t.mlnumber, ARRAY_AGG (row_to_json(t)) from(select distinct mlnumber,fromdate, todate, fromtime, totime, listprice, status "
                        ...
                        "and {3}) t "
                        "group by mlnumber) res".format(DB_WN['collection'], vendors_map[vendor], yesterday, vendors_join_filters[vendor]),
